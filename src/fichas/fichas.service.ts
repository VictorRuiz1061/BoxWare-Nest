...
export class FichasService {
  ...
  findAll(): Promise<Ficha[]> {
    return this.fichaRepository.find({ relations: ['programa', 'usuario'] });
  }

  async findOne(id: number): Promise<Ficha> {
    const ficha = await this.fichaRepository.findOne({ where: { id_ficha : id }, relations: ['programa', 'usuario'] });
    if (!ficha) {
